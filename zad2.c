...
int main(int argc, char* argv[])
{
	...
	if(proces < licznik_wierszy % zakres)						//
	{
	    start = proces * licznik_wierszy / zakres + proces;
	    koniec = (proces + 1) * licznik_wierszy / zakres + proces + 1;
	}
	else										// warunki obliczające ile wektorów powinno być przydzielone do danego procesu
	{
	    start = proces * licznik_wierszy / zakres + licznik_wierszy % zakres;
	    koniec = (proces + 1) * licznik_wierszy / zakres + licznik_wierszy % zakres;
	}										//
